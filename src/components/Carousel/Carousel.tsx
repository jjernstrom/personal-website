...
const IndexControls2 = (setIndex: React.Dispatch<React.SetStateAction<number>>): Array<ReactElement> => {
  if (catalogue)
...
export const Carousel = () => {
  ...
  const advanceIndex = () => {
    if (catalogueIndex < catalogue.length - 1) setCatalogueIndex(catalogueIndex + 1);
  ...
  return (
    <div style={{ margin: 'auto' }}>
      ...
      >
        ...
        <img src={catalogue[catalogueIndex].image} width={500} height={500} style={{ margin: '10px' }} />
        <button onClick={advanceIndex}>next</button>
      </div>

      <div style={{ display: 'flex', justifyContent: 'center' }}>{IndexControls2(setCatalogueIndex)}</div>
    </div>
  );
};
